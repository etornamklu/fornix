...
from src.database.models.radiologist import RadiologistReport
from src.database.schema.radiologist import (
    ECGReport,
    XRayReport,
    ReportType,
    CTScanReport,
...
from src.prompts.doctor_images import ECG_PROMPT, CT_PROMPT, XRAY_PROMPT, ULTRASOUND_PROMPT
...
def get_report_schema_and_prompt(report_type: ReportType):
    match report_type:
        case ReportType.xray:
            return XRayReport, XRAY_PROMPT
        case ReportType.ct_scan:
            return CTScanReport, CT_PROMPT
        case ReportType.ecg:
            return ECGReport, ECG_PROMPT
        case ReportType.ultrasound:
            return UltrasoundReport, ULTRASOUND_PROMPT
        case _:
            raise HTTPException(
                status_code=400,
...
async def stream_image_analysis(
    ...
    try:
        ...
        report = RadiologistReport(
            id=report_id,
            radiologist_id=token_data.user_id,
        ...
        for file in files:
            ...
            upload = ImageUpload(
                ...
                radiologist_report_id=report_id,
        ...
        report_schema, prompt = get_report_schema_and_prompt(report_type)
...
async def get_image_reports(
    ...
    try:
        query = db.query(RadiologistReport).filter(
                RadiologistReport.radiologist_id == token_data.user_id
        )

        if not report_type:
            # If no report type is specified, return all reports
            reports = query.order_by(desc(RadiologistReport.updated_at)).all()
        ...
        reports = (
            query.filter(RadiologistReport.type == report_type)
            .order_by(desc(RadiologistReport.updated_at))
            .all()
        )
        return [
            {
...
async def get_image_report(
    ...
    try:
        report = (
            db.query(RadiologistReport)
            .filter(
                RadiologistReport.id == report_id,
                RadiologistReport.radiologist_id == token_data.user_id,
            )
            .first()
        )
...
async def unlink_patient_from_diagnosis(
    ...
    try:
        report = (
            db.query(RadiologistReport).
            filter(RadiologistReport.id == report_id,
                   RadiologistReport.radiologist_id == token_data.user_id).
            first()
        )
