...
interface IPharmacyLandingPageProps {}

const PharmacyLandingPage = ({}: IPharmacyLandingPageProps) => {
    ...
    const features = [
        {
            ...
            icon: <FaStethoscope size={28} className="text-blue-500" />,
        ...
        <main className="w-full h-full flex flex-col items-center justify-center bg-transparent pb-5">
            <div className="w-full max-w-6xl mx-auto flex flex-col items-center justify-center gap-6 px-4 sm:px-6 md:px-8 text-center">
                ...
                <div className="flex flex-col items-center mb-6 mt-4">
                    ...
                    <p className="text-blue-600 mt-2 text-sm sm:text-base max-w-sm">
                        Revolutionizing pharmacy management for a healthier tomorrow.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                    {features.map((feature, index) => (
                        <Link
                            ...
                            className="flex flex-col items-center bg-blue-50 p-4 rounded-lg shadow-md hover:shadow-lg hover:bg-blue-100 cursor-pointer transition-all border-l-4 border-gold-400">
                            <div className="mb-3">{feature.icon}</div>
                            <h2 className="text-sm sm:text-base font-semibold text-blue-700 mb-1">{feature.title}</h2>
                            <p className="text-blue-600 text-xs sm:text-sm text-center">{feature.description}</p>
                        </Link>
                    ))}
                </div>
            </div>
